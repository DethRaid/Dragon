...
vec4 CloudColor(in vec4 worldPosition, in float sunglow, in vec3 worldLightVector)
{
	...
	else
	{
		...
		float cloudAltitudeWeight = 1.0f - clamp(distance(worldPosition.y, cloudHeight) / (cloudDepth * 0.5f), 0.0f, 1.0f);
		...
		float cloudAltitudeWeight2 = 1.0f - clamp(distance(worldPosition.y + worldLightVector.y * lightOffset * 150.0f, cloudHeight) / (cloudDepth * 0.5f), 0.0f, 1.0f);
		...
		// 	  directLightFalloff += pow(heightGradient, propigation * 0.5f);
