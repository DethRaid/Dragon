...
const float MIX_OCCLUSION_DISTANCE = 30.0;
const int   MAX_OCCLUSION_POINTS   = 16;

const int GL_EXP = 1024;
const int GL_LINEAR = 1024;
...
void main() {
	...
	float material = mix(1.0,0.4,float(mat > 0.99));
	material = mix(material,0.6,float(mat > 0.78 && mat < 0.82));
...
/* DRAWBUFFERS:0246 */
	...
	gl_FragData[1] = frag2;
	gl_FragData[2] = vec4((lmcoord.t), material, lmcoord.s, mat);
	gl_FragData[3] = vec4(specularity,0.0);
}
