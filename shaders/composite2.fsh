...
/*
 * Controlls how many rays are sent from a fragment when calculating reflctions.
 * Higher values result in smoother reflections at the cost of framerate. 2 is a
 * good nuvber to start with for a reasonable balance of runtime and image
 * quality. Higher end PCs (GTX 780) can probably handle 4, top-of-the line PCs
 * (GTX 980ti) may be able to run 16, maybe not.
 */
...
void 	CalculateSpecularReflections(inout SurfaceStruct surface) {
	...
	if (length( surface.specularColor ) > 0.00f) {
		...
		for( int i = 0; i < NUM_REFLECTION_RAYS; i++ ) {
			...
			#else
			reflection += ComputerWaterReflection( surface );
			#endif
		}
		...
		reflection.a = fakeSkyReflection.a * length( surface.specularColor );
...
void main() {
	...
	float metalness = getMetalness( texcoord.st );
	surface.specularColor = mix( vec3( 0.02 ), surface.color, metalness );
	surface.roughness = 1.0f - GetRoughness(texcoord.st);
	float ndotv = dot( surface.normal, normalize( surface.viewSpacePosition.xyz ) );
