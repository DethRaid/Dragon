...
const float desaturation = 0.0;		//this takes the colour out, 1.0 is Black/White - 0.0 is no change
...
float gen = 1.0-fract((ftime+0.5)*0.5);
	...
	vec2 Fake_Refract_1 = vec2(sin(frameTimeCounter*1.7 + texcoord.x*50.0 + texcoord.y*25.0),cos(frameTimeCounter + texcoord.y*100.0 + texcoord.x*25.0)) ;
		...
		vec3 color = GetColorTexture(texcoord.st + fake_refract * 0.005 + 0.003 * (rainlens + Fake_Refract_1*0.003));	//Sample gcolor texture
