...
/*
    float rotateAmount2 = 0.1;
    mat2 waveRotation2 = mat2(
        cos( rotateAmount2 ), -sin( rotateAmount2 ),
        sin( rotateAmount2 ), cos( rotateAmount2 )
    );
    vec3 wave2 = getNoiseTexSample(waveRotation2 * sampleCoord * vec2(1, 10)) * (windAmount * 2.5 / WIND_STRENGTH) * ;*/

vec3 getSingleWave(in vec2 samplePos, in float rotationAmount, in vec2 scale) {
...
vec3 getWaveNormal() {
    ...
    vec3 wave1 = getSingleWave(sampleCoord, -0.2, vec2(1, 100));// * (windAmount * 5 / WIND_STRENGTH);
    vec3 wave2 = getSingleWave(sampleCoord, 0.3, vec2(1, 100)) * 0.75;// * (windAmount * 2.5 / WIND_STRENGTH);
    //vec3 wave3 = getNoiseTexSample(sampleCoord * vec2(1, 10)) * (windAmount * 1.25 / WIND_STRENGTH);
    //vec3 wave4 = getNoiseTexSample(sampleCoord * vec2(1, 10)) * (windAmount * 0.75 / WIND_STRENGTH);
    //vec3 wave5 = getNoiseTexSample(sampleCoord * vec2(1, 10)) * (windAmount * 0.375 / WIND_STRENGTH);

    //newNormal.xz *= 0.01;
    newNormal = wave1 + wave2;
